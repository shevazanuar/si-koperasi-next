    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-8 rounded-3xl border border-gray-100 shadow-sm relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 relative z-10">
          <Link href="/dashboard/simpanan" className="bg-amber-600 text-white px-6 py-3 rounded-2xl font-bold text-sm hover:bg-amber-700 transition-all shadow-lg shadow-amber-500/20 active:scale-95">
            Cek Tabungan
          </Link>
        </div>
        <div className="absolute right-[-2%] top-[-20%] w-64 h-64 bg-amber-50/50 rounded-full blur-3xl pointer-events-none"></div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-12">
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between mb-10">
            ...
            <div className="bg-amber-50 px-3 py-1 rounded-lg text-[10px] font-black text-amber-600 uppercase">Live Data</div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm h-full">
            <h3 className="font-black text-gray-900 text-xs uppercase tracking-widest mb-8 border-b border-gray-50 pb-4">Transaksi Terakhir Saya</h3>
            <div className="space-y-6">
              {stats.recentActivities.length > 0 ? (
                stats.recentActivities.map((act) => (
                  <div key={act.id} className="flex gap-4 group cursor-default">
                    <div className="w-10 h-10 bg-gray-50 text-gray-400 rounded-xl flex items-center justify-center shrink-0 group-hover:bg-amber-600 group-hover:text-white transition-all">
            ...
            {stats.recentActivities.length > 0 && (
              <Link href="/dashboard/simpanan" className="w-full flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 rounded-2xl transition-all mt-8 group">
