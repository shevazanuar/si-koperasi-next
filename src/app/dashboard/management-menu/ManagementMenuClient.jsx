...
export default function ManagementMenuPage() {
  ...
  const getChildrenIds = (menuId) => {
    const menu = menus.find((m) => m.id === menuId);
    if (!menu) return [];
    return getChildren(menu.kode).map((child) => child.id);
  };
  const getParentId = (menuId) => {
    const child = menus.find((m) => m.id === menuId);
    if (!child) return null;
    const parent = menus.find(
      (m) => m.root === 0 && String(m.kode) === String(child.root),
    );
    ...
    setCheckedMenus((prev) => {
      ...
      if (isChecked) {
        const next = prev.filter(
          (id) => id !== menuId && !childrenIds.includes(id),
        );

        const parentId = getParentId(menuId);
        if (parentId) {
          const siblingIds = getChildrenIds(parentId).filter(
            (id) => id !== menuId,
          );
          const hasSiblingChecked = siblingIds.some((id) => next.includes(id));
      ...
      nextSet.add(menuId);
      childrenIds.forEach((id) => nextSet.add(id));
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Atur akses menu berdasarkan level pengguna
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="divide-y divide-gray-50">
              {levels.map((level) => (
                ...
                >
                  ...
                  <ChevronRight
                    className={`w-4 h-4 ${selectedLevel?.id === level.id ? "text-blue-500" : "text-gray-300"}`}
                  />
                </button>
              ))}
            </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
              <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider">
                {selectedLevel
                  ? `Akses Menu: ${selectedLevel.level}`
                  : "Pilih Level"}
              </h3>
              {selectedLevel && (
                <button
                  onClick={handleSave}
                  className="bg-blue-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-blue-700 transition text-sm font-medium shadow-md shadow-blue-500/20"
                >
                ...
                {rootMenus.map((m) => (
                  <div key={m.id}>
                    <label className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-gray-50 transition cursor-pointer">
                      <input
                        type="checkbox"
                        checked={checkedMenus.includes(m.id)}
                    ...
                    {getChildren(m.kode).map((child) => (
                      <label
                        key={child.id}
                        className="flex items-center gap-3 px-4 py-2.5 ml-8 rounded-xl hover:bg-gray-50 transition cursor-pointer"
                      >
                        <input
                          type="checkbox"
                          checked={checkedMenus.includes(child.id)}
