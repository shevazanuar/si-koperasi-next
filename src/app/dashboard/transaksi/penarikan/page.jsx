...
export default function PenarikanPage() {
  ...
  const filtered = anggotaList.filter((a) => a.nama.toLowerCase().includes(search.toLowerCase()) || a.nik.includes(search));
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-gray-900">Penarikan Dana</h1>
            <p className="text-sm text-gray-500">Kelola penarikan simpanan anggota</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-gray-100">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 outline-none"
              />
            </div>
          </div>
          <div className="max-h-[500px] overflow-y-auto">
            {loading ? (
              ...
              filtered.map((a) => (
                <button key={a.id} onClick={() => handleSelectAnggota(a)} className={`w-full px-4 py-3 text-left border-b border-gray-50 hover:bg-amber-50/50 transition ${selectedAnggota?.id === a.id ? "bg-amber-50" : ""}`}>
                  <p className="text-sm font-bold text-gray-900">{a.nama}</p>
                  <p className="text-xs text-gray-500">
                    NIK: {a.nik} | Saldo: Rp {fmt(a.saldo)}
                  </p>
                </button>
              ))
            )}
            ...
            <>
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      NIK: {selectedAnggota.nik} | JK: {selectedAnggota.jk}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div className="bg-amber-50 rounded-xl p-3">
                    <p className="text-xs text-gray-500">Simpanan</p>
                    <p className="font-bold text-amber-700">Rp {fmt(selectedAnggota.jml_simpanan)}</p>
                  </div>
                  <div className="bg-pink-50 rounded-xl p-3">
                    <p className="text-xs text-gray-500">Penarikan</p>
                    <p className="font-bold text-pink-700">Rp {fmt(selectedAnggota.jml_penarikan)}</p>
                  </div>
                  <div className="bg-emerald-50 rounded-xl p-3">
                    <p className="text-xs text-gray-500">Saldo</p>
                    <p className="font-bold text-emerald-700">Rp {fmt(selectedAnggota.saldo)}</p>
                  </div>
                </div>
                <button onClick={() => setShowForm(true)} className="mt-4 bg-amber-600 text-white px-4 py-2.5 rounded-xl flex items-center gap-2 hover:bg-amber-700 transition text-sm font-medium w-full justify-center">
                  + Tambah Penarikan
                </button>
              </div>
              ...
              {showForm && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                  ...
                  <form onSubmit={handleSimpan} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-amber-500"
                        />
                      </div>
                      <div>
                        ...
                        <select
                          required
                          value={form.jenis_simpanan_id}
                          onChange={(e) => {
                            setForm({ ...form, jenis_simpanan_id: e.target.value });
                            handleCheckSaldo(e.target.value);
                          }}
                          className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-amber-500"
                        >
                    ...
                    {form.jenis_simpanan_id && (
                      <div className="bg-yellow-50 border border-yellow-200 px-4 py-2 rounded-xl text-sm text-yellow-700">
                        Saldo jenis ini: <strong>Rp {fmt(saldo)}</strong>
                      </div>
                    )}
                    <div>
                      <label className="block text-xs font-semibold text-gray-500 mb-1">Jumlah Penarikan (Rp)</label>
                      <CurrencyInput required value={form.jumlah} onChange={(val) => setForm({ ...form, jumlah: val })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-amber-500" />
                    </div>
                    <div className="flex gap-2">
                      <button type="submit" className="bg-amber-600 text-white px-4 py-2.5 rounded-xl flex items-center gap-2 hover:bg-amber-700 text-sm font-medium">
                        <Save className="w-4 h-4" /> Simpan
                      </button>
                      <button type="button" onClick={() => setShowForm(false)} className="bg-gray-100 text-gray-700 px-4 py-2.5 rounded-xl flex items-center gap-2 text-sm">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
                  <h3 className="text-xs font-bold text-gray-500 uppercase">Riwayat Penarikan</h3>
                </div>
                {details.length === 0 ? (
                  <div className="p-8 text-center text-gray-400 text-sm">Belum ada penarikan.</div>
                ) : (
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {details.map((d, i) => (
                        <tr key={d.id} className="border-b border-gray-50 hover:bg-amber-50/30">
                          <td className="px-4 py-2.5 text-gray-500">{i + 1}</td>
                          <td className="px-4 py-2.5 font-mono text-amber-600 font-medium text-xs">{d.nomor}</td>
                          ...
                          <td className="px-4 py-2.5 text-center">
                            <button onClick={() => handleHapus(d.id)} className="p-1.5 hover:bg-red-50 rounded-lg text-red-500">
