...
export default function LevelAnggotaPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    const res = await fetch("/api/config/level-anggota");
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch("/api/config/level-anggota", { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    const json = await res.json();
    
    if (res.ok) {
      showSuccess("Berhasil", json.message || "Data berhasil disimpan");
      setShowForm(false); setEditId(null); setForm({ nama: "" });
      fetchData();
    } else {
  ...
  const handleDelete = async (id) => {
    const confirmed = await showConfirm("Konfirmasi Hapus", "Yakin ingin menghapus level anggota ini?");
    if (!confirmed) return;
    
    const res = await fetch(`/api/config/level-anggota?id=${id}`, { method: "DELETE" });
    const json = await res.json();
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h2 className="font-bold text-gray-900 mb-4">{editId ? "Edit" : "Tambah"} Level Anggota</h2>
        ...
        {loading ? <div className="p-12 text-center text-gray-400">Memuat...</div> : (
          <table className="w-full text-sm">
            <thead><tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="text-left px-6 py-4 text-xs font-bold text-gray-500 uppercase">Nama Level</th>
