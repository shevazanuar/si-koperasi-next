...
export default function TambahBiayaPage() {
  ...
  const [formData, setFormData] = useState({
    kode_biaya: `BOP-${new Date().toISOString().slice(0, 7).replace("-", "")}-${Math.floor(Math.random() * 1000).toString().padStart(3, "0")}`,
    nama_biaya: "",
    nominal: "",
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/biaya-opr", {
        method: "POST",
      ...
      if (res.ok) {
        showSuccess("Berhasil", "Data biaya operasional berhasil ditambahkan.");
        router.push("/dashboard/biaya");
      } else {
        showError("Gagal", result.error || "Gagal menambah data.");
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
        <div>
          <h1 className="text-xl font-black text-gray-900">Tambah Biaya Operasional</h1>
          <p className="text-sm text-gray-500">Masukkan rincian pengeluaran operasional baru</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end pt-4 border-t border-gray-100">
            <button type="submit" disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2 transition disabled:opacity-70">
